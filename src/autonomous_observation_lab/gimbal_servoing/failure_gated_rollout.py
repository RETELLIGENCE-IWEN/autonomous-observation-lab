"""Closed-loop counterfactual rollout for failure-gated position correction."""
...
from .failure_gated_policy import FailureGatedPositionCorrectionPolicy
from .gru import (
...
@dataclass
class FailureGatedCounterfactualRollout:
...
def rollout_failure_gated_position_policy(
    ...
    *,
    ...
    """Run the gated correction on its own recurrent and physical history."""

    if logged_features.ndim != 3 or logged_features.shape[-1] != len(
        FEATURE_NAMES
    ):
        raise ValueError(
            "failure-gated rollout features must have shape [batch, time, feature]"
        )
    ...
    if target_bearing_rad.shape != (batch_size, step_count + 1):
        raise ValueError("failure-gated rollout target-bearing shape is invalid")
    if time_s.shape != (batch_size, step_count + 1):
        raise ValueError("failure-gated rollout time shape is invalid")
    if capture_source_index.shape != expected:
        raise ValueError("failure-gated rollout capture-source shape is invalid")
    if sequence_mask.shape != expected:
        raise ValueError("failure-gated rollout sequence-mask shape is invalid")
    for field in fields(context):
        if getattr(context, field.name).shape != expected:
            raise ValueError(
                f"failure-gated rollout context {field.name} shape is invalid"
            )
    ...
    for time_index in range(step_count):
        ...
        command = step.command_normalized
