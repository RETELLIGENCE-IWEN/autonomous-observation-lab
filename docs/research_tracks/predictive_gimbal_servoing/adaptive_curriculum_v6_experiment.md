...
The proposed hard `integrated_midpoint` structural factorial has now been
executed as V7. Hard dynamics improved global prediction and adapter behavior,
but the selected state-only candidate failed multi-seed critical-action
replication and ensemble promotion. See the
[V7 report](midpoint_adapter_v7_experiment.md).
